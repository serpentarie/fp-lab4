...
Реализация библиотеки парсер-комбинаторов на OCaml с последующей разработкой:
1. **JSON парсера** - полная поддержка спецификации JSON
2. **Потокового CSV парсера** - эффективная обработка CSV файлов
